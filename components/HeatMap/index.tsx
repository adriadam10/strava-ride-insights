  ...
  const { groupedActivities, currentPolylines, startLatlng, currentStats, availableGridSizes } =
    useMemo(() => {
      ...
      for (const size of ALL_GRID_SIZES) {
        const groups = groupPolyline(
          activities.filter(a => a.sport_type !== 'VirtualRide' && a?.map?.summary_polyline),
          size.value
        )
          .map(group => ({
            activities: group,
            polylines: group.map(activity => activity.map.summary_polyline),
          }))
      ...
      const totalTime = currentGroup.activities.reduce(
        (sum, activity) => sum + activity.moving_time,
        0
      )
      const totalDistance = currentGroup.activities.reduce(
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-colors">
      <div className="relative aspect-[1/1] md:aspect-[16/9]">
        ...
        {mapType === 'standard' ? (
          <StandardMap key={selectedGroup} summaryPolylines={currentPolylines} />
        ) : (
          <CityRoadsMap key={selectedGroup} summaryPolylines={currentPolylines} />
        )}
      </div>
            ...
            <span className="flex items-center">
              <svg className="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <path
                  strokeLinecap="round"
                  strokeWidth="2"
                  d="M6.34 6.34l2.12 2.12 M15.54 15.54l2.12 2.12 M6.34 17.66l2.12-2.12 M15.54 8.46l2.12-2.12"
                />
              </svg>
